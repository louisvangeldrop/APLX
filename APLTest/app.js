var APLXTest = (function() {
  ...
  APLXTest.prototype.start = function() {
    ...
    var aantal =
      parms.length > 1 ? parseFloat(parms[1].replace("/", " ")) : 5e6;
    ...
    showPerformance(
      ...
      performance.now(),
      "deal",
      (dd = aantal[APLPrefix + "deal"](aantal))
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "reshape",
      (rr = aantal[APLPrefix + "reshape"](dd))
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "depth",
      (cc = dd[APLPrefix + "depth"])
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "depthLength",
      (cc = dd[APLPrefix + "depthLength"])
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "enlist",
      (dd = dd[APLPrefix + "enlist"])
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "unique",
      (dd = dd[APLPrefix + "unique"])
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "from",
      (rr = dd[APLPrefix + "from"](dd))
    );
    var apldd;
    ...
    showPerformance(
      ...
      performance.now(),
      "Array.reduceRight",
      dd.reduceRight(scalar.plus)
    );
    showPerformance(
      spanCPU,
      performance.now(),
      "Array.APLreduce",
      dd.aplReduce(scalar.minus)
    );
