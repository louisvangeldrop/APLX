...
(function (Monadic) {
    ...
    (function (NonScalar) {
        ...
        //       }
        NonScalar.inverse = (omega) => { return omega.reciprocal.divide(omega.length); };
...
(function (Dyadic) {
    var NonScalar;
    (function (NonScalar) {
        NonScalar.domino = function (omega) { return this.times(omega.inverse).aplReduce(Dyadic.Scalar.plus); };
        addPrototype([Array], 'domino', NonScalar.domino);
        NonScalar.decode = function (omega) { return omega.times(this.power(omega.length.indexGenerator.reverse())).aplReduce(Dyadic.Scalar.plus); }; // decode←{+/⍵×⍺*(⌽⍳⍴⍺)}
