namespace Monadic {
  export namespace NonScalar {
    ...
    export var ravel = (alpha): Array<any> => {
      return Array.isArray(alpha) ? alpha : [alpha];
    };
    addProperty([Array, Boolean, Date, Number, String], "ravel", ravel, false);

    export var reversed = alpha => {
      return [].concat(alpha).reverse();
    };
    addProperty([Array, Number, String], "reversed", reversed, false);

    export var shape = (alpha): number[] => {
      return alpha.length;
    };
...
namespace Dyadic {
  namespace NonScalar {
    export var catenate = function(omega) {
    ...
    export var rotate = function(omega) {
      ...
      if (myThis > 0) {
        ...
        for (counter = 0; counter < max; counter++) {
          results[counter] =
            omega[
              counter + myThis < 0 ? max + myThis + counter : counter + myThis
            ];
        }
      }
      return results;
